...
*/
	...
	$data = "select * from DinnerDescription where DinnerType = 'Rehearsal' and Enabled = 'on'";
	...
	function SetSelectedB($item)
	{
		...
		if (isset($_SESSION["RehearsalDinner"]))
		{
			...
			$a = "";
			foreach ($_SESSION['RehearsalDinner'] as $key=>$val)
			{
				$a = $a.$key;
			}
			$a = str_replace('_', ' ', $a);
...
<h1>Rehearsal or Welcome Dinner</h1>
...
<?
		for ($x = 1; $x <= 8; $x++) {
			$dataCat1 = "select * from DinnerDetail where DinnerType = 'Rehearsal' and DinnerNumber = '".$row['DinnerNumber']."' and CategoryNumber = '".$x."' and Title <> ''";
...
<?					for ($y = 1; $y <=8; $y++)
					{	
					    if ($row2['MenuItem'.$y] != "")
						{
							...
							//<?= SetSelected($i.'_'.$row2['MenuItem'.$y]) == 'Y' ? "checked='checked'" : '';
...
<br />
<h1>Rehearsal or Welcome Bar</h1>
...
<input type="hidden" value="RehearsalDinner" name="formName" />
